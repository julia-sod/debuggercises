...
function funk(parameter) {
  const local = `funky ${global}`;// funky . . hi
  global = `${global} ${global}`;// . . hi . . hi
  return `${parameter} ${local} ${global}`;// boat funky . . hi . . hi . . hi
}

function chqngeGlobql(){
  global = 'Í love bujuka girl';
}
